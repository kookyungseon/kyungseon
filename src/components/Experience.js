  ...
  return (
    <section id="experience" className="py-24 px-6 bg-gray-50 relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        <div className="absolute top-1/4 right-1/4 w-64 h-64 bg-gray-200 rounded-full opacity-30"></div>
        <div className="absolute bottom-1/4 left-1/4 w-48 h-48 bg-gray-200 rounded-full opacity-20"></div>
      </div>
            ...
            return (
              ...
              >
                <div className="relative group">
                  {/* 메인 카드 */}
                  <div className="relative bg-white border border-gray-200 p-8 md:p-12 rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 group-hover:border-gray-300">
                    {/* 헤더 */}
                    <div className="flex flex-col md:flex-row justify-between items-start mb-8">
                      <div className="flex items-center space-x-4 mb-4 md:mb-0">
                        <div className="p-4 rounded-xl bg-gray-100 border border-gray-200">
                          <IconComponent className="text-gray-600" size={32} />
                        </div>
                        <div>
                          <h3 className="text-2xl md:text-3xl font-medium text-gray-900">{exp.company}</h3>
                          <p className="text-gray-600 text-lg font-normal">{exp.position}</p>
                        </div>
                      </div>
                    ...
                    {/* 설명 */}
                    <p className="text-gray-600 text-lg leading-relaxed mb-8">{exp.description}</p>

                    {/* 성과 */}
                    <div className="mb-8">
                      <h4 className="text-xl font-medium text-gray-900 mb-6 flex items-center">
                        <CheckCircle className="text-gray-500 mr-3" size={24} />
                        주요 성과
                      </h4>
                      <div className="grid gap-4">
                        {exp.achievements.map((achievement, i) => (
                          <div key={i} className="flex items-start space-x-3 bg-gray-50 p-4 rounded-lg border border-gray-100">
                    ...
                    <div>
                      <h4 className="text-xl font-medium text-gray-900 mb-4 flex items-center">
                        <Code2 className="text-gray-500 mr-3" size={24} />
                        사용 기술
                      </h4>
                      <div className="flex flex-wrap gap-3">
                        {exp.technologies.map((tech, i) => (
                          <span
                            key={i}
                            className="px-4 py-2 bg-gray-100 border border-gray-200 text-gray-600 rounded-full text-sm font-medium hover:bg-gray-200 hover:border-gray-300 transition-all duration-300 cursor-default"
                          >
                            {tech}
                          </span>
                        ))}
